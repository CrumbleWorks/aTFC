...
public class ContinentalChunkGenerator extends ChunkGenerator {
   ...
   private ContinentalChunkGenerator(BiomeProvider biomeProviderLeft, BiomeProvider biomeProviderRight, long seed, Supplier<DimensionSettings> dimSettingsSupplier) {
      ...
      surfaceDepthNoise = (INoiseGenerator)(noisesettings.func_236178_i_() ? new PerlinNoiseGenerator(randomSeed, IntStream.rangeClosed(-3, 0)) : new OctavesNoiseGenerator(randomSeed, IntStream.rangeClosed(-3, 0)));
      randomSeed.skip(2620);
      field_236082_u_ = new OctavesNoiseGenerator(randomSeed, IntStream.rangeClosed(-15, 0));
   ...
   private double func_222552_a(int p_222552_1_, int p_222552_2_, int p_222552_3_, double p_222552_4_, double p_222552_6_, double p_222552_8_, double p_222552_10_) {
      ...
      for(int i = 0; i < 16; ++i) {
         ...
         ImprovedNoiseGenerator improvednoisegenerator = field_222568_o.getOctave(i);
         if (improvednoisegenerator != null) {
            d0 += improvednoisegenerator.func_215456_a(d4, d5, d6, d7, (double)p_222552_2_ * d7) / d3;
         }

         ImprovedNoiseGenerator improvednoisegenerator1 = field_222569_p.getOctave(i);
         if (improvednoisegenerator1 != null) {
            d1 += improvednoisegenerator1.func_215456_a(d4, d5, d6, d7, (double)p_222552_2_ * d7) / d3;
         }

         if (i < 8) {
            ImprovedNoiseGenerator improvednoisegenerator2 = field_222570_q.getOctave(i);
   ...
   private void fillNoiseColumn(double[] noiseColumn, int noiseX, int noiseZ) {
      NoiseSettings noisesettings = dimSettingsSupplier.get().getNoise();
      double d0;
      double d1;
      if (field_236083_v_ != null) {
         d0 = (double)(EndBiomeProvider.getRandomNoise(field_236083_v_, noiseX, noiseZ) - 8.0F);
         if (d0 > 0.0D) {
            d1 = 0.25D;
         } else {
            d1 = 1.0D;
      ...
      double d4 = noisesettings.func_236179_j_() ? func_236095_c_(noiseX, noiseZ) : 0.0D;
      ...
      for(int i1 = 0; i1 <= noiseSizeY; ++i1) {
         ...
         double d8 = 1.0D - (double)i1 * 2.0D / (double)noiseSizeY + d4;
         double d9 = d8 * d5 + d6;
         double d10 = (d9 + d0) * d1;
         if (d10 > 0.0D) {
            d7 = d7 + d10 * 4.0D;
   ...
   private double func_236095_c_(int p_236095_1_, int p_236095_2_) {
      double d0 = field_236082_u_.getValue((double)(p_236095_1_ * 200), 10.0D, (double)(p_236095_2_ * 200), 1.0D, 0.0D, true);
      double d1;
      if (d0 < 0.0D) {
         d1 = -d0 * 0.3D;
      } else {
   ...
   private int func_236087_a_(int p_236087_1_, int p_236087_2_, @Nullable BlockState[] p_236087_3_, @Nullable Predicate<BlockState> p_236087_4_) {
      ...
      for(int i1 = noiseSizeY - 1; i1 >= 0; --i1) {
         ...
         for(int j1 = verticalNoiseGranularity - 1; j1 >= 0; --j1) {
            ...
            BlockState blockstate = func_236086_a_(d11, k1);
            if (p_236087_3_ != null) {
   ...
   protected BlockState func_236086_a_(double p_236086_1_, int p_236086_3_) {
      ...
      } else if (p_236086_3_ < getSeaLevel()) {
               ...
               for(int l1 = verticalNoiseGranularity - 1; l1 >= 0; --l1) {
                  ...
                  for(int l2 = 0; l2 < horizontalNoiseGranularity; ++l2) {
                     ...
                     for(int k3 = 0; k3 < horizontalNoiseGranularity; ++k3) {
                        ...
                        BlockState blockstate = func_236086_a_(d18, i2);
                        if (blockstate != AIR) {
